...
function cbtText(key) {
    const lang = cbtLang();
    return (CBT_I18N[lang] && CBT_I18N[lang][key]) || CBT_I18N.ru[key] || key;
}
...
async function logMoodFromWelcome(score) {
    ...
    try {
        ...
        let msg = cbtLang() === 'en'
            ? `I would rate my mood right now as ${emojis[score] || cbtText('moodMessageDefault')} (${score}/10).`
            : `Я оцениваю свое настроение сейчас как ${emojis[score] || cbtText('moodMessageDefault')} (${score}/10).`;
...
function cleanupLegacyRhythmIntro() {
    ...
    Array.from(rhythmSection.children).forEach(function (child) {
        ...
        if (text.indexOf('Правило трёх шагов') !== -1) {
            child.remove();
        }
    });
}
...
function _renderHistDetail(r) {
    ...
    if (r.distortion === 'gratitude') {
        ...
        <div style="margin-top:12px; padding:16px; background:var(--bg); border-radius:12px; border:1px solid var(--border);">
            ...
            <div style="font-size:13px;color:var(--text);line-height:1.7;white-space:pre-line;">${r.situation}</div>
        </div>`;
    }
    if (r.entryType === 'rhythm') {
        ...
        <div style="margin-top:12px; padding:16px; background:var(--bg); border-radius:12px; border:1px solid var(--border);">
            ...
            <div style="font-size:12px;color:var(--gray);margin-bottom:12px;">${r.rhythm_summary}</div>
            <div style="display:flex;flex-direction:column;gap:8px;">
                ${items.map(function (item) {
                    return `<div style="display:flex;align-items:flex-start;gap:8px;padding:10px 12px;border:1px solid var(--border);border-radius:10px;background:var(--panel);">
                        ...
                        <span style="font-size:13px;color:var(--text);line-height:1.5;${item.done ? 'text-decoration:line-through;color:var(--gray);' : ''}">${item.text}</span>
                    </div>`;
                }).join('')}
            </div>
        </div>`;
    ...
    return header +
        ...
        (r.emotion ? `<div class="hist-detail-field">
            ...
            <div class="hist-detail-value">${r.emotion}${r.intensity ? `<span style="color:var(--gray);font-size:12px;margin-left:6px;">· ${r.intensity}/10</span>` : ''}</div>
        </div>` : '') +
        ...
        (r.distortion && r.distortion !== 'gratitude' ? `<div style="padding-top:8px;"><span style="display:inline-flex;align-items:center;gap:5px;background:var(--accent-light);color:var(--accent);border-radius:12px;padding:4px 11px;font-size:11px;">
            <i data-lucide="brain-circuit" style="width:11px;flex-shrink:0;"></i>${r.distortion}
        </span></div>` : '');
}
...
function submitPHQ(e) {
    ...
    r.innerHTML = `
                ...
                <button onclick="sendQuick('${inlineJsString(cbtLang() === 'en' ? `My PHQ-9 score is ${total} (${level}). Help me understand what to do next.` : `Мой PHQ-9 показал ${total} баллов (${level}). Помоги мне понять, что с этим делать.`)}')" 
...
function submitGAD(e) {
    ...
    r.innerHTML = `
                ...
                <div style="font-size:15px;font-weight:600;color:${color};margin-bottom:12px;">${level}</div>
                <p style="font-size:13px;color:var(--gray);line-height:1.6;">${advice}</p>
                <button onclick="sendQuick('${inlineJsString(cbtLang() === 'en' ? `My GAD-7 score is ${total} (${level}). How can I cope with this?` : `Мой GAD-7 показал ${total} баллов (${level}). Как мне справиться с этим?`)}')"
                  style="margin-top:16px;padding:10px 16px;border-radius:8px;border:1px solid var(--border);background:transparent;cursor:pointer;font-size:13px;color:var(--text);">
...
function submitEsteem(e) {
    ...
    r.innerHTML = `
                ...
                <div style="font-size:15px;font-weight:600;color:${color};margin-bottom:12px;">${level}</div>
                <p style="font-size:13px;color:var(--gray);line-height:1.6;">${advice}</p>
                <button onclick="sendQuick('${inlineJsString(cbtLang() === 'en' ? `My Rosenberg self-esteem score is ${total} (${level}). Help me understand how to improve this.` : `Моя самооценка по шкале Розенберга показала ${total} баллов (${level}). Помоги мне понять, как это улучшать.`)}')"
                  style="margin-top:16px;padding:10px 16px;border-radius:8px;border:1px solid var(--border);background:transparent;cursor:pointer;font-size:13px;color:var(--text);">
            ...
            html += `
                <div class="hist-list-item${i === selectedIdx ? ' selected' : ''}"
                    ...
                    <div style="font-size:13px;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:flex;align-items:center;gap:4px;">
                        ${r.durHrs}${cbtLang() === 'en' ? 'h' : 'ч'} <span style="color:var(--gray);">·</span> <i data-lucide="star" style="width:12px;color:var(--accent);fill:var(--accent);"></i> ${r.qual}/10
                    </div>
                </div>`;
        });
    }
    ...
    return `
        ...
        <div style="display:flex;gap:12px;margin-bottom:24px;">
            <div style="flex:1;background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;">
                <div style="font-size:24px;font-weight:700;color:var(--accent);">${r.durHrs}${cbtLang() === 'en' ? 'h' : 'ч'}</div>
                <div style="font-size:11px;color:var(--gray);">${cbtText('duration')}</div>
            </div>
            <div style="flex:1;background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center;">
                <div style="font-size:24px;font-weight:700;color:var(--accent);">${r.qual}/10</div>
        ...
        <div class="hist-detail-field">
            ...
            <div class="hist-detail-value">${r.bed} — ${r.wake}</div>
        </div>
        <div class="hist-detail-field">
            <div class="hist-detail-label">${cbtText('awakenings')}</div>
            <div class="hist-detail-value">${cbtFormat('timesCount', { count: r.awk })}</div>
        </div>
        ${r.notes ? `
        <div class="hist-detail-field">
            <div class="hist-detail-label">${cbtText('notes')}</div>
            <div class="hist-detail-value">${r.notes}</div>
        </div>` : ''}`;
}
...
const RHYTHM_SLOTS = [
    ...
    { id: 'body', icon: 'activity', title: 'Для тела', color: '#10b981', suggestions: ['Потянуться 5 минут', 'Короткая прогулка', 'Потанцевать под музыку', 'Размять плечи'], title_en: 'For the body', suggestions_en: ['Stretch for 5 minutes', 'Take a short walk', 'Dance to one song', 'Roll your shoulders'] },
    { id: 'joy', icon: 'heart', title: 'Для души', color: '#ef4444', suggestions: ['Посмотреть смешное видео', 'Почитать книгу', 'Выпить вкусный чай', 'Послушать музыку'], title_en: 'For the soul', suggestions_en: ['Watch something funny', 'Read a few pages', 'Drink a nice tea', 'Listen to music'] }
];
...
function renderActivities() {
    ...
    RHYTHM_SLOTS.forEach(function (slot) {
        ...
        var slotTitle = cbtLang() === 'en' ? slot.title_en : slot.title;
        var slotSuggestions = cbtLang() === 'en' ? slot.suggestions_en : slot.suggestions;

        if (actObj) {
            if (actObj.done) doneCount++;
            var moodBadge = '';
            if (actObj.done && actObj.mood) {
                moodBadge = '<span style="font-size:11px;color:' + (actObj.mood >= 7 ? 'var(--success)' : actObj.mood >= 4 ? 'var(--warning)' : 'var(--danger)') + ';margin-left:auto;white-space:nowrap;font-weight:600;">' + cbtText('moodAfter') + ': ' + actObj.mood + '/10</span>';
            } else if (actObj.done) {
...
function addActivityFromAI(txt) {
    ...
    div.className = 'msg assistant';
    div.innerHTML = '<div class="msg-avatar"><i data-lucide="sparkles" style="width:18px;"></i></div><div class="msg-content"><p>' + cbtFormat('addedToRhythm', { link: '<a href="#" onclick="openThoughtModalSection(\'rhythm\'); return false;" style="color:var(--accent);text-decoration:underline;">' + cbtText('rhythmLink') + '</a>', text: txt }) + '</p></div>';
    if (btnMessage) {
        btnMessage.appendChild(div);
        if (window.lucide) window.lucide.createIcons();
        btnMessage.scrollTop = btnMessage.scrollHeight;
...
function renderCalendar() {
    ...
    document.getElementById('calDayTitle').innerText = window.t ? window.t('calendar_pick_day') : 'Выберите день';
...
function calSelectDay(key, day) {
    ...
    if (thoughts.length) {
        ...
        thoughts.forEach(t => {
            ...
            html += `<div style="border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:10px;background:var(--bg);">
                        ...
                        <div style="margin-bottom:6px;"><span style="font-size:11px;text-transform:uppercase;color:var(--gray);">${cbtText('fieldSituation')}</span><br>${t.situation}</div>
                        <div style="margin-bottom:6px;"><span style="font-size:11px;text-transform:uppercase;color:var(--gray);">${cbtText('fieldEmotion')}</span><br>${t.emotion} — ${t.intensity}/10</div>
                        <div style="margin-bottom:6px;"><span style="font-size:11px;text-transform:uppercase;color:var(--gray);">${cbtText('fieldBalancedResponse')}</span><br>${t.rational_response}</div>
                    </div>`;
        });
    }
        ...
        moodLog.forEach(m => {
            ...
            html += `<div style="border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:10px;background:var(--bg);">
                        ...
                        <div style="display:flex; align-items:center; gap:8px; font-weight:500;">
                            ...
                            <span>${cbtFormat('moodScore', { score: m.score })}</span>
                        </div>
                    </div>`;
        });
    }
        ...
        slLog.forEach(s => {
            html += `<div style="border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:10px;background:var(--bg);">
                        <div style="display:flex; justify-content:space-between; margin-bottom: 6px;">
                            <span>${cbtText('bedtime')}: <b>${s.bed}</b></span>
                            <span>${cbtText('wakeup')}: <b>${s.wake}</b></span>
                        </div>
                        <div style="margin-bottom: 6px; font-size:13px;">${cbtFormat('sleepDurationQuality', { hours: s.durHrs, quality: s.qual })}</div>
    ...
    if (tests.length) {
        ...
        tests.forEach(t => {
            html += `<div style="border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:10px;background:var(--bg);">
                        <div style="font-weight:600;margin-bottom:4px;">${t.name}: ${t.score} ${cbtText('points')}</div>
...
function renderDashboard() {
    ...
    document.getElementById('dashCards').innerHTML = `
                ...
                <div class="dash-card" style="border-left: 3px solid #797298;">
                    <div class="dash-card-val">${avgSleep}${avgSleep !== '—' ? (cbtLang() === 'en' ? 'h' : 'ч') : ''}</div>
    ...
    if (phqHist.length) {
        ...
        testHtml += `<div style="display:flex;justify-content:space-between;align-items:center;padding:12px;border-radius:10px;border:1px solid var(--border);margin-bottom:8px;">
                    <div><div style="font-weight:600;margin-bottom:2px;">${cbtText('phqTitle')}</div><div style="font-size:12px;color:var(--gray);">${last.level} · ${new Date(last.date).toLocaleDateString(cbtLocale())}</div></div>
    ...
    if (gadHist.length) {
        ...
        testHtml += `<div style="display:flex;justify-content:space-between;align-items:center;padding:12px;border-radius:10px;border:1px solid var(--border);">
                    <div><div style="font-weight:600;margin-bottom:2px;">${cbtText('gadTitle')}</div><div style="font-size:12px;color:var(--gray);">${last.level} · ${new Date(last.date).toLocaleDateString(cbtLocale())}</div></div>
    ...
    if (esteemHist.length) {
        ...
        testHtml += `<div style="display:flex;justify-content:space-between;align-items:center;padding:12px;border-radius:10px;border:1px solid var(--border);margin-top:${testHtml ? '8px' : '0'};">
                    <div><div style="font-weight:600;margin-bottom:2px;">${cbtText('esteemTitle')}</div><div style="font-size:12px;color:var(--gray);">${last.level} · ${new Date(last.date).toLocaleDateString(cbtLocale())}</div></div>
                    <div style="text-align:right;"><div style="font-size:22px;font-weight:700;color:${col};">${last.score}<span style="font-size:13px;font-weight:400;color:var(--gray)">/30</span></div>
                    ${prev !== null ? `<div style="font-size:11px;color:${last.score > prev ? '#797298' : '#df5858'}">${trend}</div>` : ''}</div></div>`;
    }
